...
func (s *StoreSuite) TestLogInsert() {
	...
	err := s.Store.SmsRepository.InsertSMSLog(newLog)
	s.NoError(err)
...
func (s *StoreSuite) TestLogGet() {
	...
	receivedLog, err := s.Store.SmsRepository.GetSMSLog(&newLog)
...
func (s *StoreSuite) TestUpdateLog() {
	...
	err = s.Store.SmsRepository.UpdateSMSLog(newLog)
	s.NoError(err)

	updatedLog, err := s.Store.SmsRepository.GetSMSLog(&newLog)
	s.NoError(err)
